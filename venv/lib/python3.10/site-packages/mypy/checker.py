...
from mypy import message_registry, nodes, operators
...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
                    ...
                    if typ.arg_kinds[i] == nodes.ARG_STAR:
                        if isinstance(arg_type, ParamSpecType):
                            ...
                            arg_type = self.named_generic_type(
                                "builtins.tuple", [arg_type]
                            )
                    elif typ.arg_kinds[i] == nodes.ARG_STAR2:
                        if (
                            not isinstance(arg_type, ParamSpecType)
                            and not typ.unpack_kwargs
                        ):
                            arg_type = self.named_generic_type(
    ...
    def check_enum_bases(self, defn: ClassDef) -> None:
        ...
        for base in defn.info.bases:
            if enum_base is None and base.type.is_enum:
                ...
                self.fail(
                    f'No non-enum mixin classes are allowed after "{enum_base}"', defn
                )
                break
    ...
    def visit_expression_stmt(self, s: ExpressionStmt) -> None:
        ...
        if error_note_and_code:
            ...
            self.fail(
                message_registry.TYPE_MUST_BE_USED.format(format_type(expr_type)),
                s,
                code=code,
            )
    ...
    def analyze_iterable_item_type(self, expr: Expression) -> tuple[Type, Type]:
        ...
        if isinstance(iterable, TupleType):
            ...
            return (
                iterator,
                echk.check_method_call_by_name("__next__", iterator, [], [], expr)[0],
            )
    ...
    def _check_for_truthy_type(self, t: Type, expr: Expression) -> None:
        ...
        if isinstance(t, FunctionLike):
            self.fail(
                message_registry.FUNCTION_ALWAYS_TRUE.format(format_type(t)), expr
            )
...
def is_overlapping_types_no_promote_no_uninhabited(left: Type, right: Type) -> bool:
    ...
    return is_overlapping_types(
        left, right, ignore_promotions=True, ignore_uninhabited=True
    )
