...
try:
    ...
    if sys.version_info >= (3, 8):
        ...
        from ast import AST, Attribute, Bytes, Call
        from ast import Ellipsis as ast3_Ellipsis
        from ast import Expression as ast3_Expression
        from ast import (
            FunctionType,
            Index,
            Name,
        ...
        from typed_ast.ast3 import AST, Attribute, Bytes, Call
        from typed_ast.ast3 import Ellipsis as ast3_Ellipsis
        from typed_ast.ast3 import Expression as ast3_Expression
...
def parse(
    ...
    try:
        ...
        tree = ASTConverter(options=options, is_stub=is_stub_file, errors=errors).visit(
            ast
        )
        tree.path = fnam
...
class TypeConverter:
    ...
    def _extract_argument_name(self, n: ast3.expr) -> str | None:
        if isinstance(n, Str):
            ...
            return None
        self.fail(
            f"Expected string literal for argument name, got {type(n).__name__}",
            self.line,
            0,
        )
        return None
