...
def tuple_fallback(typ: TupleType) -> Instance:
    ...
    for item in typ.items:
        if isinstance(item, UnpackType):
            ...
            items.append(item)
    return Instance(
        info, [join_type_list(items)], extra_attrs=typ.partial_fallback.extra_attrs
    )
...
def bind_self(
    method: F, original_type: Type | None = None, is_classmethod: bool = False
) -> F:
...
def _remove_redundant_union_items(items: list[Type], keep_erased: bool) -> list[Type]:
    ...
    for i, item in enumerate(items):
        ...
        for j, tj in enumerate(items):
            ...
            if is_redundant_literal_instance(
                proper_item, proper_tj
