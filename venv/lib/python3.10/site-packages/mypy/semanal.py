...
class SemanticAnalyzer(
        ...
        for name, t in implicit_module_attrs.items():
            if name == "__doc__":
                ...
                sym = self.lookup_qualified(
                    "__builtins__.dict", Context(), suppress_errors=True
                )
                if not sym:
                    continue
                node = sym.node
    ...
    def analyze_class(self, defn: ClassDef) -> None:
        ...
        declared_metaclass, should_defer = self.get_declared_metaclass(
            ...
            if param_name == "covariant":
                ...
                try:
                    ...
                    if isinstance(upper_bound, AnyType) and upper_bound.is_from_error:
                        self.fail(
                            message_registry.TYPEVAR_BOUND_MUST_BE_TYPE, param_value
                        )
    ...
    def process_typevartuple_declaration(self, s: AssignmentStmt) -> bool:
        ...
        if len(call.args) > 1:
            self.fail(
                "Only the first argument to TypeVarTuple has defined semantics", s
            )
...
def is_trivial_body(block: Block) -> bool:
    ...
    if (
        ...
        and isinstance(body[0].expr, StrExpr)
    ):
        body = block.body[1:]
