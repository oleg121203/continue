...
class LowLevelIRBuilder:
    def __init__(
        self, current_module: str, mapper: Mapper, options: CompilerOptions
    ) -> None:
    ...
    def coerce_int_to_fixed_width(
        ...
        if size < int_rprimitive.size:
            ...
            check3 = self.add(
                ComparisonOp(src, Integer(-upper_bound, src.type), ComparisonOp.SGE)
            )
    ...
    def binary_op(self, lreg: Value, rreg: Value, op: str, line: int) -> Value:
        ...
        if is_fixed_width_rtype(ltype):
            if op in FIXED_WIDTH_INT_BINARY_OPS:
                ...
                if isinstance(rreg, Integer):
                    return self.comparison_op(
                        lreg, Integer(rreg.value >> 1, ltype), op_id, line
                    )
            ...
            if (
                ...
                if isinstance(lreg, Integer):
                    return self.comparison_op(
                        Integer(lreg.value >> 1, rtype), rreg, op_id, line
                    )
                return self.comparison_op(lreg, rreg, op_id, line)
    ...
    def is_different_native_int_signs(
